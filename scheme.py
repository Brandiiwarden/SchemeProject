...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        operands = rest.map(lambda rest: scheme_eval(rest, env))
...
def eval_all(expressions, env):
    ...
    if not scheme_eval(expressions.first, env):
...
class Frame(object):
    ...
    def lookup(self, symbol):
        ...
        if symbol not in self.bindings and self.parent:
            self = self.parent
            return Frame.lookup(self, symbol)
        # END PROBLEM 2
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target): 
        ...
        instance = LambdaProcedure(formals, expressions.rest, env)
        env.define(name, instance)
        return name
        # END PROBLEM 9
    else:
...
def do_and_form(expressions, env):
    ...
    "*** YOUR CODE HERE ***"
    if expressions is nil:
        return is_true_primitive(expressions)
    ...
    if is_false_primitive(expressions):
        return False
    evaluated = eval_all(expressions, env)
...
def do_or_form(expressions, env):
    ...
    "*** YOUR CODE HERE ***"
    if expressions is nil:
        return is_false_primitive(expressions)
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if is_true_primitive(test):
            ...
            if clause.first == True and clause.rest is nil:
                return True

            while clause.rest is not nil:
                if clause:
                    return eval_all(clause.rest, env)
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        evaled_values = scheme_eval(bindings.first.rest.first, env)
        values = Pair(evaled_values, values)
